...
/**
 * Frame do swing, usado para visualizar e alterar valores do sistema.
 *
 */
public class Atualizar extends JFrame {
	...
	/**
	 * Transforma um double x para uma string d.
	 * 
	 * @param x
	 * @return d
	 */
	...
	/**
	 * Inicializa as variáveis desta frame.
	 */
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (!hTFValoresMatInf.showingHint() && hTFValoresMatInf.getText().length() != 0)
						if (hTFValoresMatInf.checaValor()) {
							...
							errorMsg += "Valor da matr�cula para Ensino Infantil mal formatado. ";
							printError = true;
						}
					...
					if (!hTFValoresMatFund.showingHint() && hTFValoresMatFund.getText().length() != 0)
						if (hTFValoresMatFund.checaValor()) {
							...
							errorMsg += "Valor da matr�cula para Ensino Fundamental mal formatado. ";
							printError = true;
						}
					...
					if (!hTFValoresParcInf.showingHint() && hTFValoresParcInf.getText().length() != 0)
						if (hTFValoresParcInf.checaValor()) {
							...
							errorMsg += "Valor da parcela para Ensino Inf�ntil mal formatado. ";
							printError = true;
						}
